...
export const TaskDialogContext =
  createContext<ITaskDialogContext>(initialValue);

export const TaskDialogContextProvider: React.FC<React.ReactNode> = ({
  children,
}) => {
  ...
  const handleTaskChange = (name: string, value: string) => {
    setTaskDialog({
      ...taskDialog,
      task: { ...taskDialog.task, [name]: value },
    });
  };
